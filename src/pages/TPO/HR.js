    ...
    return (
      <section className="relative py-4 px-4 md:px-8">
        ...
        <div className="flex bg-white h-screen">
          <div className="flex flex-col space-y-3 w-2/5 overflow-y-auto border-r-2 p-4">
            {messages.map((item, index) => (
              <div
                key={index}
                className="w-auto flex flex-row border-2 rounded-lg space-x-4 p-5 cursor-pointer"
                onClick={() => {
                  rtx(item);
          ...
          <div className="w-3/5 p-4 overflow-y-auto">
            <div ref={emailRef} className="w-full h-full p-4 whitespace-pre-wrap border-2 rounded-lg"></div>
          </div>
        </div>
      </section>
    );
